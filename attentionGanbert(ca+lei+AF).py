...
# =========================
# Unlabeled U1 gate (keep 3-loss structure)
# =========================
USE_U1_ENTROPY_GATE = True     # 只改 D_L_unsupervised1U 的计算方式
U1_GATE_K = 5.0                # sigmoid 陡峭度，越小越平滑(更稳)
TAU_INIT = 0.7                 # tau_c 初始值(0~1)，越大越宽松
TAU_EMA_ALPHA = 0.05           # tau_c EMA 平滑系数
Z_EMA_GAMMA = 0.1              # 分母(有效权重和)EMA 平滑系数

# ramp-up：对原来的无监督两项(U1,U2)做系数调度（不新增损失项）
U_RAMPUP_EPOCHS = 10           # 前10个epoch逐步放开
U_LAMBDA_MAX = 1.0             # 放开到的最大系数（想更稳可用0.7）
...
def allTrain(epochs, transformer, generator, discriminator, train_dataloader, test_dataloader, device, label_list,
             ...
             detail_log_path: str = None, fold_id: int = None, rho_per_class = None):
    val_f1s = []
    ...
    # ===== U1 熵门控状态：tau_c (按类阈值) + Z_ema(有效权重和EMA) =====
    tau_c = None
    Z_ema = None
    if USE_U1_ENTROPY_GATE:
        # C = 真实类别数（不含 fake 列），与你的 logits = D_real_logits[:, :-1] 一致
        C = len(label_list)  # 这里 label_list 就是 LABEL_LIST
        tau_c = torch.full((C,), float(TAU_INIT), device=device)
        if rho_per_class is None:
            rho_per_class = np.full((C,), 0.6, dtype=np.float32)  # 兜底：统一分位数
    ...
    for epoch_i in range(0, epochs):
        ...
        if USE_U1_ENTROPY_GATE:
            C = len(label_list)
            entropy_bucket = [[] for _ in range(C)]
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # =========================
            #  Unsupervised U1: real not fake (entropy-gated, EMA-normalized)
            # =========================
            p_fake_real = D_real_probs[:, -1]  # [B]
            u1_term = -torch.log(1.0 - p_fake_real + EPS)  # [B]

            if USE_U1_ENTROPY_GATE:
                # 仅对 unlabeled 样本做门控；labeled 权重固定为1，保持监督稳定
                unl_mask = (~b_label_mask.bool()).to(device)  # [B]
                weights = torch.ones_like(u1_term)  # [B]

                if unl_mask.any():
                    logits_real = D_real_logits[:, :-1]  # [B, C]
                    p_real = torch.softmax(logits_real, dim=-1)  # [B, C]
                    # 归一化熵 Hbar in [0,1]
                    H = -(p_real * torch.log(p_real + EPS)).sum(dim=-1)  # [B]
                    C = p_real.size(-1)
                    Hbar = H / math.log(C)  # [B]

                    # 预测类别（仅用于取 tau_c，不做伪标签CE）
                    y_hat = torch.argmax(p_real, dim=-1)  # [B]
                    tau_hat = tau_c[y_hat]  # [B]

                    w_u = torch.sigmoid(U1_GATE_K * (tau_hat - Hbar))  # [B]
                    weights[unl_mask] = w_u[unl_mask]

                    # 本 epoch 统计用：把 unlabeled 的 (y_hat, Hbar) 存起来，epoch 末更新 tau_c
                    if 'entropy_bucket' not in locals():
                        entropy_bucket = [[] for _ in range(C)]
                    yh_cpu = y_hat[unl_mask].detach().cpu().numpy()
                    hb_cpu = Hbar[unl_mask].detach().cpu().numpy()
                    for yy, hh in zip(yh_cpu, hb_cpu):
                        entropy_bucket[int(yy)].append(float(hh))

                Z_t = weights.sum()  # 当前 batch 有效权重和
                if Z_ema is None:
                    Z_ema = Z_t.detach()
                else:
                    Z_ema = (1.0 - Z_EMA_GAMMA) * Z_ema + Z_EMA_GAMMA * Z_t.detach()

                D_L_unsupervised1U = (weights * u1_term).sum() / (Z_ema + EPS)
            else:
                D_L_unsupervised1U = u1_term.mean()

            # U2 保持不变（fake is fake）
            D_L_unsupervised2U = -1 * torch.mean(torch.log(D_fake_probs[:, -1] + EPS))

            # =========================
            # ramp-up: 只调度原有无监督两项(U1,U2)，不新增损失类别
            # =========================
            if U_RAMPUP_EPOCHS > 0:
                lam_u = min(1.0, float(epoch_i + 1) / float(U_RAMPUP_EPOCHS)) * float(U_LAMBDA_MAX)
            else:
                lam_u = float(U_LAMBDA_MAX)

            d_loss = D_L_Supervised + lam_u * D_L_unsupervised1U + lam_u * D_L_unsupervised2U
        ...
        # ===== epoch end: update tau_c by class-quantile + EMA smoothing =====
        if USE_U1_ENTROPY_GATE:
            tau_new = tau_c.detach().cpu().numpy()  # 当前 tau
            for c in range(len(tau_new)):
                if len(entropy_bucket[c]) == 0:
                    continue
                rho_c = float(rho_per_class[c])
                q = float(np.quantile(np.array(entropy_bucket[c], dtype=np.float32), rho_c))
                tau_new[c] = (1.0 - TAU_EMA_ALPHA) * tau_new[c] + TAU_EMA_ALPHA * q

            tau_c = torch.tensor(tau_new, device=device, dtype=torch.float32)
...
def run_single_experiment(dataset_name, data_dir,
    ...
    for fold_id in range(n_splits):
        ...
        # ====== labeled 的类别频次（用于 rho_per_class 设计）======
        label_indices = [label_map[ex[1]] for ex in labeled]
        class_counts = np.bincount(label_indices, minlength=num_labels)
        max_n = max(1, int(class_counts.max()))

        # ====== 类别分位数 rho_c：少数类更宽松，保证不断流（稳定 macro）======
        # rho_c = clip(0.5 + 0.1 * log(max_n / n_c), 0.5, 0.9)
        rho_per_class = np.zeros(num_labels, dtype=np.float32)
        for c in range(num_labels):
            n_c = max(1, int(class_counts[c]))
            q = max_n / n_c
            rho = 0.5 + 0.1 * np.log(q)
            rho_per_class[c] = float(np.clip(rho, 0.5, 0.9))
        ...
        best_epoch, best_metrics = allTrain(
            epochs, transformer, generator, discriminator, train_dataloader, test_dataloader, device, LABEL_LIST,
            gen_optimizer, dis_optimizer, SA1_fusion, CA_fusion, ce_weights,
            scheduler_d, scheduler_g, detail_file, fold_id + 1,
            rho_per_class
        )
